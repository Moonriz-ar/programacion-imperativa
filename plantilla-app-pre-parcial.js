...
const inmobiliaria = {
  // A array de departamentos
  departamentos,
  // B metodo listarDepartamentos
  listarDepartamentos: function (arrayDepartamento) {
    for (let i = 0; i < arrayDepartamento.length; i++) {
      let departamento = arrayDepartamento[i];
      let verificarDisponibilidad = departamento.disponible
        ? "Disponible"
        : "Alquilado";
      let verificarMascotas = departamento.aceptaMascotas
        ? "acepta mascotas"
        : "no acepta mascotas";
      console.log(
        `id: ${departamento.id}, precio $ ${departamento.precioAlquiler}, está ${verificarDisponibilidad}, ${departamento.cantidadHabitacion} ambientes, máximo ${departamento.cantidadPersonas} personas, ${verificarMascotas}, ${departamento.propietarios}`
      );
    }
  },
  // C metodo departamentosDisponibles
  departamentosDisponibles: function () {
    let listadoDepartamentosDisponibles = [];
    for (let i = 0; i < this.departamentos.length; i++) {
      let departamento = this.departamentos[i];
      if (departamento.disponible) {
        listadoDepartamentosDisponibles.push(departamento);
      }
    }
    return listadoDepartamentosDisponibles;
  },
  // D metodo buscar por id
  buscarPorID: function (idDepartamento) {
    ...
    return `No se pudo encontrar ningún departamento con el id ${idDepartamento}`;
  },
  // E metodo buscarPorPrecio
  buscarPorPrecio: function (maxPrice) {
    let departamentosDisponibles = inmobiliaria.departamentosDisponibles();
...
// Ejecución aquí
console.log(inmobiliaria.buscarPorID(6));

console.log(v, oo + "  E. buscarPorPrecio");
// Ejecución aquí
console.log(inmobiliaria.buscarPorPrecio(2));
